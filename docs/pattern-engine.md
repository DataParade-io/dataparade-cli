...
- `language`: `"typescript"` | `"javascript"` | `"python"` | `"go"` | `"php"` | `"rust"` | `"ruby"` | `"java"` | `"kotlin"` | `"cpp"` | `"csharp"` (the engine branches on this)
...
- `moduleLevelCalls?`: call sites with `{ callee, argumentsSnippet, location }` (used by Python, Go, PHP, Rust, Ruby, JVM, C++, and C# external API matching)
- `types?`: declared types with `{ name, kind?, baseTypes?, decorators?, location }` (used by C# controller/DbContext detection)
- `strippedContent?`: source with comments blanked out and layout preserved, supplied by the C-family, PHP, Rust, and Ruby analyzers so line-based rules never match commented-out code
...
- Rust routes/serverless/db/auth/env/config/external APIs: `cli/patterns/rust.patterns.yaml`
- Ruby routes/serverless/db/auth/env/config/external APIs: `cli/patterns/ruby.patterns.yaml`
...
4. Let property-detection (`getPropertiesFromFinding`) enrich component properties.
   (In this repo, the TypeScript/JavaScript, Python, Go, PHP, Rust, Ruby, Java/Kotlin, C++, and C# analyzers all apply this YAML-driven enrichment step.)

...
describe('ai-enrichment candidate selector', () => {
  it('selects third-party, sparse property, flow direction, and missing interaction candidates', () => {
    ...
    const candidateTypes = candidates.map(
  ...
  it('puts Tier 1 property keys first in bounded missingFields', () => {
    const properties = Object.fromEntries(
      Array.from({ length: 40 }, (_, index) => [
        `z_field_${String(index).padStart(2, '0')}`,
        null,
      ])
    );
    properties.encrypt_at_rest = null;
    properties.authentication_method = null;
    ...
    const propertyCandidate = candidates.find(
      (candidate) => candidate.candidateType === 'node_property'
    );
    expect(propertyCandidate?.missingFields.slice(0, 2)).toEqual([

...
export function mapRawLogToActions(
  rawLogPath: string,
  interviewId = "datap-676-mapped-from-raw",
): SimulatedInterview {
  ...
  return {
    id: interviewId,
    description:
      "Mechanical mapper output from raw-model-transcript.jsonl (DATAP-676). Not hand-edited.",
    actions,
  };
}
...
const REQUIRED_SKILL_SHA = "05a6a5ba4c4e82b6db3ead1add4bbda10c0caf55";

export function assertRunMetaSkillSha(rawLogPath: string): void {
  const lines = fs.readFileSync(rawLogPath, "utf8").split("\n").filter(Boolean);
  const meta = lines
    .map((line) => JSON.parse(line) as RawLogEvent)
    .find((event) => event.type === "run_meta");
  if (!meta) {
    throw new Error(`No run_meta event in ${rawLogPath}`);
  }
  if (meta.skillSha !== REQUIRED_SKILL_SHA) {
    throw new Error(
      `Expected run_meta.skillSha ${REQUIRED_SKILL_SHA}; got ${String(meta.skillSha)}`,
    );
  }
}

export function assertResponseModelIsLuna(rawLogPath: string): void {
  const required = "gpt-5.6-luna";
  const lines = fs.readFileSync(rawLogPath, "utf8").split("\n").filter(Boolean);
  const responseModels = lines
    .map((line) => JSON.parse(line) as RawLogEvent)
    .filter((event) => event.type === "api_response")
    .map((event) => event.model);

  if (responseModels.length === 0) {
    throw new Error(`No api_response events in ${rawLogPath}`);
  }
  for (const responseModel of responseModels) {
    if (typeof responseModel !== "string" || !responseModel.includes(required)) {
      throw new Error(
        `Expected api_response.model to include ${required}; got ${String(responseModel)}`,
      );
    }
  }
}

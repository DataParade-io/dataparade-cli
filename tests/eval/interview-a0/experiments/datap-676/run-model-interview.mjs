#!/usr/bin/env node
/**
 * DATAP-676: live OpenAI interview run with gpt-5.6-luna.
...
const model = process.env.DATAP_676_MODEL ?? REQUIRED_MODEL;
const maxTurns = Number(process.env.DATAP_676_MAX_TURNS ?? "120");

if (model !== REQUIRED_MODEL) {
  throw new Error(`DATAP-676 requires model id ${REQUIRED_MODEL}, got ${model}`);
}
...
async function main() {
  ...
  appendLog({
    ...
    skillSha: "05a6a5ba4c4e82b6db3ead1add4bbda10c0caf55",
  });

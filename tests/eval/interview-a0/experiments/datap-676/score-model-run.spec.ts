...
describe("DATAP-676 model-run experiment (gpt-5.6-luna)", () => {
  beforeAll(() => {
    if (!fs.existsSync(rawLogPath)) {
      throw new Error(
        `Missing ${rawLogPath}. Run: OPENAI_API_KEY=... node tests/eval/interview-a0/experiments/datap-676/run-model-interview.mjs`,
      );
    }
    assertResponseModelIsLuna(rawLogPath);
    assertRunMetaSkillSha(rawLogPath);
    mapRawLogToInterviewFile(rawLogPath, mappedPath, "datap-676-mapped-from-raw");
  });
  ...
  it("maps raw API log mechanically and scores six named rubric lines", () => {
    ...
    expect(report.interviewId).toBe("datap-676-mapped-from-raw");

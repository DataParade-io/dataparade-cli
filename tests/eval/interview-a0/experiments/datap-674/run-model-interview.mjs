#!/usr/bin/env node
/**
 * DATAP-674: live OpenAI interview run with gpt-5.6-luna.
...
const model = process.env.DATAP_674_MODEL ?? REQUIRED_MODEL;
const maxTurns = Number(process.env.DATAP_674_MAX_TURNS ?? "120");

if (model !== REQUIRED_MODEL) {
  throw new Error(`DATAP-674 requires model id ${REQUIRED_MODEL}, got ${model}`);
}

...
describe("DATAP-673 model-run experiment", () => {
  beforeAll(() => {
    if (!fs.existsSync(rawLogPath)) {
      throw new Error(
        `Missing ${rawLogPath}. Run: OPENAI_API_KEY=... node tests/eval/interview-a0/experiments/datap-673/run-model-interview.mjs`,
      );
    }
    mapRawLogToInterviewFile(rawLogPath, mappedPath, "datap-673-mapped-from-raw");
  });
  ...
  it("maps raw API log mechanically and scores six named rubric lines", () => {
    ...
    expect(report.interviewId).toBe("datap-673-mapped-from-raw");

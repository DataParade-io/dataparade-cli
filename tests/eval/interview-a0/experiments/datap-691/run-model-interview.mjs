#!/usr/bin/env node
/**
 * DATAP-691: live OpenAI interview run with gpt-5.6-luna (skill pin 2616ebc).
...
const model = process.env.DATAP_691_MODEL ?? REQUIRED_MODEL;
const maxTurns = Number(process.env.DATAP_691_MAX_TURNS ?? "120");

if (model !== REQUIRED_MODEL) {
  throw new Error(`DATAP-691 requires model id ${REQUIRED_MODEL}, got ${model}`);
}

...
describe("DATAP-691 model-run experiment (gpt-5.6-luna, skill 2616ebc)", () => {
  beforeAll(() => {
    if (!fs.existsSync(rawLogPath)) {
      throw new Error(
        `Missing ${rawLogPath}. Run: OPENAI_API_KEY=... node tests/eval/interview-a0/experiments/datap-691/run-model-interview.mjs`,
      );
    }
    assertResponseModelIsLuna(rawLogPath);
    assertRunMetaSkillSha(rawLogPath);
    mapRawLogToInterviewFile(rawLogPath, mappedPath, "datap-691-mapped-from-raw");
  });
  ...
  it("maps raw log, scores rubric, and exports interview Discoveries (adapter only)", () => {
    ...
    expect(report.interviewId).toBe("datap-691-mapped-from-raw");
    ...
    const exportBundle = exportInterviewDiscoveries(mapped, {
      ...
      rawEvidenceRef: "transcript:experiments/datap-691/raw-model-transcript.jsonl",
    });

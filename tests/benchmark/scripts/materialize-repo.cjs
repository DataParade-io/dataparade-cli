#!/usr/bin/env node
/**
 * Compiled wrapper for materialize-repo — imports from dist/ so we don't need
 * --experimental-strip-types. Used by `pnpm run benchmark:materialize` after build.
 */
const fs = require("fs");
...
function usage() {
  console.log(
    "Usage: node tests/benchmark/scripts/materialize-repo.js <repo-key> | --all",
  );

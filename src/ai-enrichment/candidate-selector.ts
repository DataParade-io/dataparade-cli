...
export function selectInferenceCandidates(input: {
  ...
  for (const component of input.components) {
    ...
    if (component.type !== 'actor' && hasSparseProperties(component)) {
      const sparseFields = sortPropertyKeysByPriority(
  ...
  for (const flow of input.dataFlows) {
    ...
    const suspiciousDirection =
      source.type !== 'actor' && target.type === 'actor';
      ...
      candidates.push(
        buildCandidate(
          ...
          'Flow involving actor has low confidence or suspicious direction.',
          {
            flowId: flow.id,
            hints: [
              'verify caller->callee direction from route and handler evidence',
            ],
          }
        )
      );
  ...
  if ((input.sections?.length ?? 0) >= 2 && crossSectionFlows === 0) {
    candidates.push(
      buildCandidate(
        ...
        {
          hints: [
            'search for API clients, fetch/axios, and backend route wiring',
          ],
        }
      )
    );

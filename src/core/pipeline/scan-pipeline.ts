    ...
    return {
      ...
      targetComponentId:
        proposal.kind === 'component_patch'
          ...
          : undefined,
      targetFlowId:
        proposal.kind === 'flow_patch' ? proposal.targetFlowId : undefined,
      sourceComponentId:
...
async function runScanPipelineInner(
  ...
  dataFlows.splice(
    0,
    dataFlows.length,
    ...structuralInference.mergeResult.dataFlows
  );
  ...
  structuralEnrichmentSummary = {
    ...
    proposalsRejected:
      structuralInference.mergeResult.rejectedProposalIds.length,
  };
  ...
  if (config.enableAiInference) {
    ...
    components.splice(
      0,
      components.length,
      ...inference.mergeResult.components
    );
    ...
    const genP = inference.proposals.filter((p) =>
      p.id.startsWith('provider_')
    );
    const appliedP = appliedIds.filter((id) =>
    ...
    const providerTargetNotFound =
      rejectionCounts.get('provider:target_not_found') ?? 0;
    ...
    aiInferenceSummary = {
      ...
      aiProvider: orchOptions.platformProxy
        ? 'dataparade'
        : orchOptions.provider,
  ...
  const fallbackResult = applyDeterministicInferenceFallbacks(
    components,
    dataFlows
  );
  ...
  const redFlagPass = applyConnectivityRedFlagPass(components, finalizedFlows);
  components.splice(0, components.length, ...redFlagPass.components);
  finalizedFlows = redFlagPass.dataFlows;
  const redFlags: ScanRedFlag[] = redFlagPass.redFlags;
  for (const flag of redFlags) {
    warnings.push(formatRedFlagWarning(flag));
  }
